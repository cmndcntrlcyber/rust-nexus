...
impl BOFLoader {
    ...
    fn build_symbol_table(
        ...
        for (i, (_sym_idx, name_opt, symbol)) in coff.symbols.iter().enumerate() {
            let name: String = match name_opt {
                Some(s) => s.to_string(),
                None => match symbol.name(&coff.strings) {
                    Ok(s) => s.to_string(),
                    Err(_) => format!("sym_{}", i),
                },
            };
            ...
            let address = if symbol.section_number == 0 {
                ...
                if let Some(&api_addr) = self.api_resolver.get(name.as_str()) {
                    api_addr
                } else {
            ...
            symbols.insert(name, bof_symbol);
        }
    ...
    fn apply_relocations(
        ...
        coff_data: &[u8],
        _symbols: &HashMap<String, BofSymbol>,
    ) -> InfraResult<()> {
        for section in &coff.sections {
