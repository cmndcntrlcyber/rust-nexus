...
impl BOFLoader {
    ...
    fn init_api_resolver(&mut self) {
        ...
        info!("Initialized API resolver with {} functions", self.api_resolver.len());
    }
    ...
    pub fn load_bof(&self, coff_data: &[u8]) -> InfraResult<LoadedBof> {
        ...
        info!("BOF loaded successfully at {:p}, entry point: 0x{:x}", base_address, entry_point);
        Ok(loaded_bof)
    }
    ...
    fn calculate_image_size(&self, coff: &Coff) -> InfraResult<usize> {
        ...
        if total_size == 0 || total_size > 50 * 1024 * 1024 { // 50MB limit
            return Err(InfraError::BofError("Invalid or excessive image size".to_string()));
        }
    ...
    fn allocate_memory(&self, size: usize) -> InfraResult<*mut u8> {
        ...
        unsafe {
            ...
            if ptr.is_null() {
                return Err(InfraError::BofError("Failed to allocate memory".to_string()));
            }
            Ok(ptr)
        }
    }
    ...
    fn load_sections(&self, coff: &Coff, base_address: *mut u8, coff_data: &[u8]) -> InfraResult<()> {
        for section in &coff.sections {
            ...
            debug!("Loaded section '{}' at offset 0x{:x} ({} bytes)",
    ...
    fn apply_relocations(&self, coff: &Coff, base_address: *mut u8, coff_data: &[u8], symbols: &HashMap<String, BofSymbol>) -> InfraResult<()> {
        for section in &coff.sections {
            ...
            if relocations_start + relocations_size > coff_data.len() {
                return Err(InfraError::BofError("Relocation data out of bounds".to_string()));
            }
            
            // Parse relocations (simplified - would need full implementation for production)
            debug!("Processing {} relocations for section '{}'", 
    ...
    fn make_executable(&self, base_address: *mut u8, size: usize) -> InfraResult<()> {
        unsafe {
            ...
            if result == 0 {
                return Err(InfraError::BofError(
                    format!("Failed to make memory executable: {}", GetLastError())
                ));
            }
        }
    ...
    pub fn execute_bof(&self, loaded_bof: &LoadedBof, function_name: &str, args: &[BofArgument]) -> InfraResult<String> {
        info!("Executing BOF function: {} with {} arguments", function_name, args.len());
        
        // Find the function symbol
        let function_symbol = loaded_bof.symbols.get(function_name)
            .ok_or_else(|| InfraError::BofError(format!("Function '{}' not found", function_name)))?;
        
        if !function_symbol.is_function || function_symbol.is_imported {
            return Err(InfraError::BofError(format!("Symbol '{}' is not a valid function", function_name)));
        }
    ...
    fn call_function(&self, function_address: usize, arg_buffer: &[u8]) -> InfraResult<String> {
        ...
        info!("Calling function at 0x{:x} with {} bytes of arguments", function_address, arg_buffer.len());
        ...
        unsafe {
            ...
            Ok(format!("BOF executed successfully, return value: {}", result))
        }
    }
    
    #[cfg(not(target_os = "windows"))]
    fn call_function(&self, _function_address: usize, _arg_buffer: &[u8]) -> InfraResult<String> {
        Err(InfraError::BofError("BOF execution not supported on this platform".to_string()))
    }
...
mod tests {
    ...
    fn test_argument_marshalling() {
        let loader = BOFLoader::new();
        let args = vec![
            BofArgument::int32(123),
            BofArgument::string("test"),
        ];
