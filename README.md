...
[`docs/enhancements/agent-swarm-mesh/v1.0/shell-session-protocol.md`](docs/enhancements/agent-swarm-mesh/v1.0/shell-session-protocol.md);
for the v1.2 API surface see
[`docs/enhancements/agent-swarm-mesh/v1.2/migration-from-v1.1.md`](docs/enhancements/agent-swarm-mesh/v1.2/migration-from-v1.1.md).

---

## Deployment scripts

All deployment operations are handled by scripts in [`scripts/`](scripts/).

### Cert generation

| Script | Purpose |
|---|---|
| [`scripts/gen-certs.sh`](scripts/gen-certs.sh) | Dev/test CA + certs (localhost only) |
| [`scripts/gen-certs-prod.sh`](scripts/gen-certs-prod.sh) | Production CA + server/operator/agent certs with correct SANs |
...
| Script | Purpose |
|---|---|
| [`scripts/transfer-prep.sh`](scripts/transfer-prep.sh) | Stages the server binary, certs, config, and a one-shot setup script into `scripts/transfer/` ready to SCP to a remote host |
...
| Script | Purpose |
|---|---|
| [`scripts/build-agent-bundles.sh`](scripts/build-agent-bundles.sh) | Builds the agent binary, mints per-host client certs, and packages into zip bundles |
| [`scripts/deliverables-prep.sh`](scripts/deliverables-prep.sh) | One-command wrapper — builds Linux and/or Windows bundles with configurable counts |
...
| Script | Purpose |
|---|---|
| [`scripts/deploy-operator-console.sh`](scripts/deploy-operator-console.sh) | Builds the Tauri console, wires cert env vars, and launches |
| [`scripts/operator-package.sh`](scripts/operator-package.sh) | Packages the console binary + operator certs into a distributable folder (with optional `--zip`) |
...
- **CI codesigning** for macOS + Windows Tauri bundles — see
  ...
  and [`docs/enhancements/agent-swarm-mesh/v1.2/codesigning.md`](docs/enhancements/agent-swarm-mesh/v1.2/codesigning.md).

The complete defense matrix and threat model is at
[`docs/enhancements/agent-swarm-mesh/v1.2/security-overview.md`](docs/enhancements/agent-swarm-mesh/v1.2/security-overview.md).
...
| Generate production certs | [`scripts/gen-certs-prod.sh`](scripts/gen-certs-prod.sh) |
| Stage server files for remote host | [`scripts/transfer-prep.sh`](scripts/transfer-prep.sh) |
| Build + package agent bundles | [`scripts/deliverables-prep.sh`](scripts/deliverables-prep.sh) |
| Package operator console | [`scripts/operator-package.sh`](scripts/operator-package.sh) |
| v1.2 security overview | [`docs/enhancements/agent-swarm-mesh/v1.2/security-overview.md`](docs/enhancements/agent-swarm-mesh/v1.2/security-overview.md) |
| v1.2 migration notes (from v1.1) | [`docs/enhancements/agent-swarm-mesh/v1.2/migration-from-v1.1.md`](docs/enhancements/agent-swarm-mesh/v1.2/migration-from-v1.1.md) |
| Tauri codesigning (CI) | [`docs/enhancements/agent-swarm-mesh/v1.2/codesigning.md`](docs/enhancements/agent-swarm-mesh/v1.2/codesigning.md) |
| Wire reference (shell-session protocol) | [`docs/enhancements/agent-swarm-mesh/v1.0/shell-session-protocol.md`](docs/enhancements/agent-swarm-mesh/v1.0/shell-session-protocol.md) |
